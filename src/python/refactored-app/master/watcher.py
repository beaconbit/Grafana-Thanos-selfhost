...
class WatcherThread(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.manage_device_threads(registry)
            self.recheck_invalid_devices = False
    ...
    def manage_device_threads(self, registry):
# ************* ACCESS SHARED DEVICE REGISTRY ******************
        logger.info("Registry found")
        current_devices = registry.get_all_devices_copy()
        for device in current_devices:
            mac = device["mac"]
            logger.debug("Checking device {mac} is still valid")
    ...
    def start_worker_for_device(self, device):
        mac = device["mac"]
        thread = DeviceWorker(device)
        thread.start()
